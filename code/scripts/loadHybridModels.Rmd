...
paramdir = sprintf("%scode/fit_models/hybrid/rl/ss/",basedir)
...
ctrl_fact_map = pd.read_csv(os.path.join(r.basedir,"data/controlHybridData.csv"))[["subject_id","sub_factor"]]
...
pat_fact_map = pd.read_csv(os.path.join(r.basedir,"data/patientHybridData4Stan.csv"))[["subject_id","sub_factor"]]
...
for (s in 1:18) {
  f = readRDS(sprintf("%shybrid_rand_ss_fit_pat_%s.rds",paramdir,s))
...
for (s in 1:54) {
  f = readRDS(sprintf("%shybrid_rand_ss_fit_ctrl_%s.rds",paramdir,s))
  f_sum = as.data.frame(summary(f, pars = c("beta","post_pred"))$summary)
  f_sum$sub_factor = s
  f_sum$group = "CTRL"
  f_sum = cbind(param = rownames(f_sum), f_sum)
...
for (s in 1:18) {
  f = readRDS(sprintf("%shybrid_rw_ss_fit_pat_%s.rds",paramdir,s))
...
for (s in 1:54) {
  f = readRDS(sprintf("%shybrid_rw_ss_fit_ctrl_%s.rds",paramdir,s))
  f_sum = as.data.frame(summary(f, pars = c("alpha","beta","post_pred"))$summary)
  f_sum$sub_factor = s
  f_sum$group = "CTRL"
  f_sum = cbind(param = rownames(f_sum), f_sum)
...
ctrl_rands = ctrl_rands[ctrl_rands.sub_factor != 10] # remove subj A2OQMY85Q2WY68
pat_rws = r.rw_fit_params[r.rw_fit_params.group == "PAT"]
ctrl_rws = r.rw_fit_params[r.rw_fit_params.group == "CTRL"]
ctrl_rws = ctrl_rws[ctrl_rws.sub_factor != 10] # remove subj A2OQMY85Q2WY68
...
# exclude control subject who responded red on almost all trials
fit_params = fit_params[fit_params.subject_id != 'A2OQMY85Q2WY68']
fit_params.to_csv(os.path.join(r.outdir,"full_hybrid_rw_model_params.csv"))
```
...
# patient
pb_params = beta1_params[beta1_params.group_coded == -0.5]
# controls
cb_params = beta1_params[beta1_params.group_coded == 0.5]

```

```{r}
beta1_m = stan_lmer(mean ~ group_coded + (group_coded | patient_factor), data = py$beta1_params)
...
beta1_table.to_csv(os.path.join(r.outdir,"decksensitivity_stat_effects.csv"))
```

```{r}
py$pb_params["ds_bwd"] = as.vector(scale(py$pb_params$ds_bwd))
beta1_m = stan_glm(mean ~ ds_bwd, data = py$pb_params)
beta1_table = data.frame(summary(beta1_m, probs=c(0.975,0.025,0.9,0.1),digits=3))
```
...
beta1_table = r.beta1_table
beta1_table["effect"] = beta1_table.index
beta1_table.to_csv(os.path.join(r.outdir,"decksensitivity_PAT_stat_effects.csv"))
```

```{r}
py$cb_params["ds_bwd"] = as.vector(scale(py$cb_params$ds_bwd))
beta1_m = stan_glm(mean ~ ds_bwd, data = py$cb_params)
beta1_table = data.frame(summary(beta1_m, probs=c(0.975,0.025,0.9,0.1),digits=3))
```
...
beta1_table = r.beta1_table
beta1_table["effect"] = beta1_table.index
beta1_table.to_csv(os.path.join(r.outdir,"decksensitivity_CTRL_stat_effects.csv"))
```
...
# patient
pb_params = beta2_params[beta2_params.group_coded == -0.5]
# controls
cb_params = beta2_params[beta2_params.group_coded == 0.5]

```

```{r}
beta2_m = stan_lmer(mean ~ group_coded + (group_coded | patient_factor), data = py$beta2_params)
...
beta2_table.to_csv(os.path.join(r.outdir,"oldbias_stat_effects.csv"))
```
...
beta1_table = r.beta1_table
beta1_table["effect"] = beta1_table.index
beta1_table.to_csv(os.path.join(r.outdir,"oldbias_PAT_stat_effects.csv"))
```
...
beta1_table = r.beta1_table
beta1_table["effect"] = beta1_table.index
beta1_table.to_csv(os.path.join(r.outdir,"oldbias_CTRL_stat_effects.csv"))
```
...
# patient
pb_params = beta3_params[beta3_params.group_coded == -0.5]
# controls
cb_params = beta3_params[beta3_params.group_coded == 0.5]

```

```{r}
beta3_m = stan_lmer(mean ~ group_coded + (group_coded | patient_factor), data = py$beta3_params)
...
beta3_table.to_csv(os.path.join(r.outdir,"oldsensitivity_stat_effects.csv"))
```

```{r}
py$pb_params["ds_bwd"] = as.vector(scale(py$pb_params$ds_bwd))
beta3_m = stan_glm(mean ~ ds_bwd, data = py$pb_params)
beta3_table = data.frame(summary(beta3_m, probs=c(0.975,0.025,0.9,0.1),digits=3))
```
...
beta3_table = r.beta3_table
beta3_table["effect"] = beta3_table.index
beta3_table.to_csv(os.path.join(r.outdir,"oldsensitivity_PAT_stat_effects.csv"))
```

```{r}
py$cb_params["ds_bwd"] = as.vector(scale(py$cb_params$ds_bwd))
beta3_m = stan_glm(mean ~ ds_bwd, data = py$cb_params)
beta3_table = data.frame(summary(beta3_m, probs=c(0.975,0.025,0.9,0.1),digits=3))
```
...
beta3_table = r.beta3_table
beta3_table["effect"] = beta3_table.index
beta3_table.to_csv(os.path.join(r.outdir,"oldsensitivity_CTRL_stat_effects.csv"))
```

# Load and save the ELPD LOO performance for each model/subject

```{r}
model_names = c("rand","hybrid")
cdf_df = data.frame()
for (s in 1:18) {
  loo1 = readRDS(sprintf("%shybrid_rand_ss_pat_%s_loo.rds",paramdir,s))
  loo2 = readRDS(sprintf("%shybrid_rw_ss_pat_%s_loo.rds",paramdir,s))
...
for (s in 1:54) {
  loo1 = readRDS(sprintf("%shybrid_rand_ss_ctrl_%s_loo.rds",paramdir,s))
  loo2 = readRDS(sprintf("%shybrid_rw_ss_ctrl_%s_loo.rds",paramdir,s))
...
elpd_diff_ctrl = (np.array(np.abs(elpds_ctrl[elpds_ctrl.model_name == "hybrid"]["elpd"])) - np.array(np.abs(elpds_ctrl[elpds_ctrl.model_name == "rand"]["elpd"])))*-1
elpd_diff_pat = (np.array(np.abs(elpds_pat[elpds_pat.model_name == "hybrid"]["elpd"])) - np.array(np.abs(elpds_pat[elpds_pat.model_name == "rand"]["elpd"])))*-1
elpd_diffs = pd.DataFrame.from_dict({"elpd_diff":[i for j in [elpd_diff_ctrl,elpd_diff_pat] for i in j],
                                     ...
                                     "sub_factor":elpds[elpds.model_name == "hybrid"]["sub_factor"]})
...
elpd_diffs = elpd_diffs[elpd_diffs.subject_id != 'A2OQMY85Q2WY68']
```

```{python}
r.cdf_df.to_csv(os.path.join(r.outdir,"hybrid_model_loo_df.csv"))
elpd_diffs.to_csv(os.path.join(r.outdir,"hybrid_elpd_diffs.csv"))
```
